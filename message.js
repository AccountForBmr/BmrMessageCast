...
var messageCast = function() {
    ...
    var _messageList = {
        ...
        "InsertStutterSpeech": `\${MESSAGECAST.loadCustomSpeech();MESSAGECAST.addSpeechRule(/(\\b([^\\s])[^\\s]*\\b)+/gm, (...arguments)=>{let lustModifier = STATUS.player.lust||0.05; let stutterChance = 0.9*lustModifier;if(Math.random()<stutterChance){console.log("stuttering");let stutterPart = arguments[1]+"-";let numberOfStutters = 1; let additionalStutterChance=Math.random(); if(additionalStutterChance < 0.5){numberOfStutters+=1;}if(additionalStutterChance < 0.15){numberOfStutters+=1;}let actualStutter="";for(let i = 0; i< numberOfStutters;i++){actualStutter+=stutterPart;}return actualStutter+arguments[1];}else{console.log("not stuttering");return arguments[1];}});}`,
        /*Change Character*/
